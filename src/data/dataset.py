...
class SoundscapeDataset(Dataset):
    ...
    def __init__(
        ...
        # Deduplicate: each (filename, start) pair appears twice in the CSV,
        # keep only the first occurrence
        before = len(self.labels_df)
        self.labels_df = self.labels_df.drop_duplicates(subset=["filename", "start"], keep="first")
        self.labels_df = self.labels_df.reset_index(drop=True)
        print(f"[SoundscapeDataset] Dropped {before - len(self.labels_df)} duplicate rows "
              f"({before} -> {len(self.labels_df)})")
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, str]:
        ...
        if soundscape_path.exists():
            spec = self.processor.process_fixed_window(
...
if __name__ == "__main__":
    ...
    spec, labels, row_id = dataset[0]
    print(f"Spectrogram shape: {spec.shape}, Labels shape: {labels.shape}, Row ID: {row_id}")